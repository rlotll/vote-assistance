...
- 다크모드 대응 색상은 두 종류로 분리:
  - **고정 색상** (브랜드, 사전투표, 투표용지 색): 라이트/다크 동일
  - **시맨틱 색상** (텍스트, 배경): `@media (prefers-color-scheme: dark)`에서 재정의
...
| 토큰 | 값 | 용도 (와이어프레임 §3.2) |
|---|---|---|
| `--color-brand` | `#1D9E75` | 버튼, 활성 탭, 링크, 현재 진행 스텝 |
...
| `--color-background-secondary` | `#020617` |
| `--color-border-default` | `rgba(241, 245, 249, 0.12)` |
| `--color-focus-ring` | `#5DCAA5` |

> 다크모드에서 브랜드/사전투표/투표용지 색상은 **유지**한다 (식별성·중립성). 가독성은 텍스트 색상에서 보장.
